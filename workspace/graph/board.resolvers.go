...
func (r *mutationResolver) UpdateBoard(ctx context.Context, id string, input model.UpdateBoard) (*model.Board, error) {
	...
	err = r.db.Collection(existingBoard.Collection()).FindOne(ctx, filter).Decode(existingBoard)
	if err != nil {
		return nil, err
	}
	...
	if input.Description != nil {
		existingBoard.Description = input.Description
	}
	if input.IsTemplate != nil {
		existingBoard.IsTemplate = *input.IsTemplate
	}
	if input.Order != nil {
		existingBoard.Order = *input.Order
	}
	if input.Starred != nil {
		existingBoard.Starred = *input.Starred
	}
	existingBoard.UpdatedBy = uid

	// Perform the update in the database
	update := bson.M{
		"$set": existingBoard,
	}
	_, err = r.db.Collection(existingBoard.Collection()).UpdateOne(ctx, filter, update)
	if err != nil {
		return nil, err
	}
