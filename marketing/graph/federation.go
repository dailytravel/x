	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		case "Campaign":
			...
			case "findCampaignByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findCampaignByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindCampaignByID(ctx, id0)
				if err != nil {
		...
		case "Link":
			...
			case "findLinkByID":
				id0, err := ec.unmarshalNID2string(ctx, rep["id"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findLinkByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindLinkByID(ctx, id0)
				if err != nil {
