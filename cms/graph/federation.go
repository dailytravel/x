...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		case "Template":
			resolverName, err := entityResolverNameForTemplate(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Template": %w`, err)
			}
			switch resolverName {
			...
			case "findTemplateByID":
				id0, err := ec.unmarshalNID2string(ctx, rep["id"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findTemplateByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindTemplateByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Template": %w`, err)
				}
...
func entityResolverNameForTemplate(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
		...
		return "findTemplateByID", nil
	}
	return "", fmt.Errorf("%w for Template", ErrTypeNotFound)
}
