...
func (r *queryResolver) Portfolios(ctx context.Context, stages map[string]interface{}) (*model.Portfolios, error) {
	pipeline := bson.A{}
	...
	cursor, err := r.db.Collection("portfolios").Aggregate(ctx, pipeline)
	if err != nil {
		return nil, err
	}
	defer cursor.Close(ctx)
